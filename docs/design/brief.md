# Design brief — Soft Paper (portfolio UI)

| Item | Choice |
|------|--------|
| Screens | Dashboard `/` + Review `/review/[jobId]` |
| Impression | Soft Paper — practical, editorial, light |
| Density | 4 (airy but tool-like) |
| Light/dark | Light only |
| Font | IBM Plex Sans JP |
| Icons | None added (type + rules only) |
| Signature | Left ink rule on primary sections; hairline dividers |

Tokens: see `frontend/src/app/globals.css` and [design.md](../design.md) §2.

---

# デザインブリーフ — Soft Paper（ポートフォリオ UI）

| 項目 | 選択 |
|------|------|
| 画面 | ダッシュボード `/` + レビュー `/review/[jobId]` |
| 印象 | Soft Paper — 実務的・紙面・ライト |
| 密度 | 4（余白あり、道具寄り） |
| 明暗 | ライトのみ |
| フォント | IBM Plex Sans JP |
| アイコン | 追加なし（文字と罫線のみ） |
| シグネチャ | 主要セクションの左インク線、ヘアライン区切り |

トークン: `frontend/src/app/globals.css` と [design.md](../design.md) §2。
