...
| モード | 用途 | 本リポジトリでの扱い |
|--------|------|----------------------|
| **Portfolio**（デモ環境） | 提案・検証・デモンストレーション | Mock API + フロント + 設計 docs |
| **Production**（本番環境） | 実際の業務運用 | Playwright / 候補マッチング / Sheets 実装（非公開可） |
...
- API キー・サービスアカウントは環境変数のみ。デモ環境（portfolio）のリポジトリに本番キーを含めない。

### 3.4 運用・可観測性

- ジョブステータスは DB と API で一貫して参照可能。
- 失敗ジョブはリトライ上限後 DLQ（本番環境 Phase 2）。
...
| # | 決定 | MVP での要件 |
|---|------|----------------|
| 1 | **デモ環境（portfolio）を先行** | `APP_MODE=portfolio` を既定とし、Mock で E2E デモ可能であること |
