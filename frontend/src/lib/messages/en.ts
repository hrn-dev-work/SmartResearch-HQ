import type { Messages } from "@/lib/messages/types";

export const en: Messages = {
  meta: {
    title: "SmartResearch HQ",
    description:
      "Match Shopee listings to Amazon ASINs. Confirm on the review screen.",
  },
  header: {
    aboutDemo: "About demo",
  },
  localeToggle: {
    ja: "JA",
    en: "EN",
    switchToJa: "Switch to Japanese",
    switchToEn: "Switch to English",
  },
  dashboard: {
    title: "Shopee research",
    description:
      "Paste a Shopee shop URL. SOLD items and Amazon ASIN candidates appear on the review screen.",
    shopUrlLabel: "Shopee shop URL",
    shopUrlPlaceholder: "https://shopee.sg/shop/...",
    displayNameLabel: "Display name",
    displayNameOptional: "Optional",
    displayNamePlaceholder: "e.g. Demo shop",
    submit: "Start research",
    submitting: "Starting…",
    errorStart: "Failed to start research",
    metricManualLabel: "Manual",
    metricManualLine: "5–10 min per item",
    metricToolLabel: "This tool",
    metricToolLine: "~10 sec per item (est.)",
    metricModeLabel: "Mode",
    metricModeLine: "Mock (portfolio)",
  },
  review: {
    back: "← Back",
    title: "Review",
    itemCount: "{count} items",
    export: "Export to sheet",
    exporting: "Exporting…",
    loading: "Loading…",
    processing: "{pct}% in progress",
    processingHint:
      "Research running ({pct}%). The item list appears when done.",
    empty: "No items to review.",
    errorLoadItems: "Failed to load items",
    errorExport: "Export failed",
    confirmed: "Confirmed: {title}",
    rejected: "Rejected: {title}",
    confirmedAsin: "Confirmed with ASIN {asin}: {title}",
    exportResult: "Exported {exported} ({skipped} skipped)",
    shopee: "Shopee",
    amazonCandidates: "Amazon candidates",
    soldCount: "Sold {count}",
    openShopee: "Open on Shopee",
    noCandidates: "No candidates. Enter an ASIN.",
    confidence: "{asin} · {pct}% match",
    select: "Select",
    manualAsin: "Manual ASIN",
    confirmAsin: "Confirm this ASIN",
    submitting: "Saving…",
    reject: "No match (reject)",
    decided: "Confirmed",
    errorDecide: "Failed to save decision",
  },
  status: {
    PENDING: "Pending",
    SCRAPING: "Scraping",
    SCRAPE_FAILED: "Scrape failed",
    AI_INFERENCE: "Matching",
    AI_FAILED: "Match failed",
    AWAITING_REVIEW: "Awaiting review",
    APPROVED: "Approved",
    REJECTED: "Rejected",
    EXPORTED: "Exported",
  },
  asin: {
    length: "ASIN must be 10 characters (B + 9 alphanumeric)",
    format: "Use B followed by 9 alphanumeric characters",
  },
  about: {
    title: "About this demo",
    subtitle: "SmartResearch-HQ — Portfolio mode",
    close: "Close",
    purposeHeading: "Purpose",
    purposeBody:
      "Cross-border listing research (Shopee, etc.): scrape, suggest ASINs, human review, then export to a spreadsheet.",
    archHeading: "Public vs ops build",
    archIntro:
      "One codebase runs Portfolio (public demo) or Production (ops). You are on Portfolio.",
    archBulletMock: "No Postgres or Redis — in-memory mock (APP_MODE=portfolio)",
    archBulletFixture:
      "Fixed fixtures run end-to-end without infra cost or secrets",
    prodHeading: "Production mode (reference)",
    prodIntro: "Live deployment adds (disabled in this demo):",
    prodBulletScrape: "Playwright scraping of Shopee SOLD items",
    prodBulletQueue: "Async jobs via Redis + ARQ",
    prodBulletMatch: "Pluggable matching (Amazon PA-API, Gemini, etc.)",
    prodBulletSheets: "Bulk export via Google Sheets API",
  },
};
